...
export const metadata: Metadata = {
  title: "مرکز مهارت‌های زبان انگلیسی | ایندورا",
...
const FEATURED_ARTICLES = [
  ...
  },
  {
    ...
    readMinutes: 8,
    isPremium: true,
    author: "لابراتوار آکوستیک ایندورا",
  },
  {
...
export default function SkillsHubPage() {
  return (
    <PublicShell locale="fa" currentPath="/skills">
      <div className={styles.container} dir="rtl">
        <header className={styles.heroHeader}>
          <span className={styles.heroBadge}>پایگاه دانش یادگیری زبان</span>
          <h1 className={styles.heroTitle}>مهارت‌های زبان انگلیسی ایندورا</h1>
        ...
        <section className={styles.noticeBanner} aria-label="شفافیت آموزشی">
          ...
          <div>
            ...
            <p style={{ margin: 0, marginTop: "0.25rem" }}>
              تمام محتواهای آموزشی این مرکز به‌صورت مستقل و توسط اساتید ایندورا تدوین شده‌اند و هیچ‌گونه کپی‌برداری از منابع تجاری ثالث وجود ندارد.
            </p>
          </div>
        </section>
        ...
        <section className={styles.articlesSection} aria-label="مقالات منتخب">
          <h2 style={{ fontSize: "var(--font-size-h2)", fontWeight: 800, marginBottom: "var(--space-4)" }}>
            مقالات و راهنماهای آموزشی برگزیده
          </h2>
            ...
            {FEATURED_ARTICLES.map((art) => (
              <article key={art.slug} className={styles.articleCard}>
                <div className={styles.articleMeta}>
                  ...
                  <span style={{ color: "var(--color-text-muted)" }}>⏱️ {art.readMinutes} دقیقه مطالعه</span>
                  {art.isPremium && (
                    <span style={{ color: "var(--color-achievement-amber)", fontWeight: 700 }}>★ ویژه</span>
                  )}
                </div>
                <h3 className={styles.articleTitle}>{art.titleFa}</h3>
                <div style={{ color: "var(--color-text-muted)", fontFamily: "var(--font-family-latin)", direction: "ltr", textAlign: "right", fontSize: "var(--font-size-small)", marginBottom: "var(--space-2)" }}>
                  {art.titleEn}
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--color-border)", paddingTop: "var(--space-3)" }}>
                  <span style={{ fontSize: "var(--font-size-small)", color: "var(--color-text-muted)" }}>✍️ {art.author}</span>
                  <Link
