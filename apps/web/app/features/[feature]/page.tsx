...
export async function generateMetadata({ params }: { params: Promise<{ feature: string }> }) {
  ...
  const copy = featurePages[key].fa;
  return buildMetadata("fa", `/features/${key}`, copy.title, copy.summary);
}

export default async function FeatureRoute({ params }: { params: Promise<{ feature: string }> }) {
  const { feature } = await params;
  if (!featureKeys.includes(feature as FeatureKey)) notFound();
  return <FeaturePage locale="fa" featureKey={feature as FeatureKey} />;
}
